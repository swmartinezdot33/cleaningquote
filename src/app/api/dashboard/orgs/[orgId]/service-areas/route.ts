...
export async function GET(
  ...
  if (user) {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/cfb75c6a-ee25-465d-8d86-66ea4eadf2d3', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'orgs/[orgId]/service-areas:GET', message: 'GHL branch', data: { orgId, hasRequestLocationId: !!requestLocationId, hasSessionLocationId: !!ghlSession?.locationId, resolvedLocationId: !!locationId }, timestamp: Date.now(), hypothesisId: 'H2-H3' }) }).catch(() => {});
    // #endregion
    if (locationId) {
  ...
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/cfb75c6a-ee25-465d-8d86-66ea4eadf2d3', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'orgs/[orgId]/service-areas:GET', message: 'access check', data: { orgId, hasRequestLocationId: !!requestLocationId, allowed }, timestamp: Date.now(), hypothesisId: 'H3' }) }).catch(() => {});
  // #endregion
  if (!allowed) {
...
/** POST - Create service area. Body: { name, polygon? } or { name, kmlContent? } or { name, network_link_url? }. Auth: Supabase user or GHL iframe (locationId from request). */
export async function POST(
  ...
  const requestLocationId = locationIdFromRequest(request);
  if (user) {
    canManage = await canManageOrg(user.id, user.email ?? undefined, orgId);
  } else {
    const ghlSession = await getSession();
    const locationId = requestLocationId ?? ghlSession?.locationId ?? null;
    if (locationId) {
      canManage = await canAccessOrgViaGHLLocation(orgId, locationId);
      if (canManage) client = createSupabaseServer();
    }
  }
  if (!canManage) {
    return NextResponse.json(
      user ? { error: 'Only org admins can create service areas' } : { error: 'Unauthorized' },
      { status: user ? 403 : 401 }
    );
  }
